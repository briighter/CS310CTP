...
public class StudentTest {
	...
	public void testGetStudentNameList3() {                                             //Added by William Leischner
		...
		StudentList studentList = new StudentList();									//instantiate the StudentList object so we can access it's methods and properties
		studentNames = studentList.getStudentsNames();									//populate the studentNames list with the actual values in the StudentsList object
		assertEquals("William Scott Leischner", studentNames.get(2));							//test case to see if the second value contains the name we expect
	}
	
	@Test
	public void testGetStudentNameList4() {
		List<String> studentNames = new ArrayList<String>();							//Created variable Jerrod Willbrandt for student list of names
		StudentList studentList = new StudentList();									//instantiate the StudentList object so we can access it's methods and properties
		studentNames = studentList.getStudentsNames();									//populate the studentNames list with the actual values in the StudentsList object
		assertEquals("Jerrod Willbrandt", studentNames.get(3));							//test case to see if the third value contains the name Jerrod Willbrandt
	}
